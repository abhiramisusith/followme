...
const getFollow = payload => apis.get(`/Follow?FollowUser_Id=${payload.id}`, { headers: { "Authorization": `Bearer ${payload.token}` } })
const getUnFollow = payload => apis.get(`/UnFollow?UnFollowUser_Id=${payload.id}`, { headers: { "Authorization": `Bearer ${payload.token}` } })
